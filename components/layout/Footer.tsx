...
const quickLinks = [
  ...
  { href: '/how-we-record/', label: 'How We Record' },
...
// Social icons keyed by SOCIAL_PROFILES name. Hrefs come from the shared source
// (lib/site-social.ts) so the footer and schema sameAs can never drift. The
// Google Maps link below is a UI convenience (not a sameAs entry) and stays.
const SOCIAL_ICONS = {
  Facebook: (
...
const socialLinks = [
  ...SOCIAL_PROFILES.map((p) => ({
    name: p.name,
    href: p.url,
    ariaLabel: `Follow us on ${p.name}`,
    icon: SOCIAL_ICONS[p.name],
  })),
  {
